...
import { FaHome, FaBook, FaBars, FaPhone } from 'react-icons/fa'; // Exemplo de ícones do Font Awesome
    ...
    <header className={`header ${isVisible ? "visible" : "hidden"}`}>
      ...
      <nav>
        ...
        <div className={`icon_container ${isMenuOpen ? "open" : ""}`}>
            ...
            <button className="icon-button" onClick={() => scrollToSection('history')}>
              <FaBook size={28} /> {/* Ícone de Search */}
            </button>
            <button className="icon-button" onClick={() => scrollToSection('contacts')}>
              <FaPhone size={28} /> {/* Ícone de User */}
            </button>
        </div>
        <div className="menu-toggle" onClick={() => setIsMenuOpen(!isMenuOpen)}>
        <button>
              <FaBars size={28} /> {/* Ícone de User */}
            </button>
        </div>
      </nav>
    </header>
